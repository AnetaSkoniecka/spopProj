...
getPossibleFtype :: Plaster -> Field -> [FieldType]
getPossibleFtype p f @ (Field ft oft fx fy) = do
	let nfts = [ t | (Field t o x y) <- getNeighbours p f]
	let allftypes = [Empty, A, B, C, D, E, F, G]
	[a | a <- allftypes, not (elem a (ft:nfts))]

solveWithPrepare :: Plaster -> Plaster
solveWithPrepare p = solve p (getAllFields p)

solveFor (p @ (Plaster rows size backtrack)) ((f @ (Field ftype oftype x y)):fs) (pftype:pftypes) = do
	let ftype = pftype
	let p @ (Plaster rows size backtrack) = solve p fs
	if backtrack then do
		let ftype = oftype
		solveFor p (f:fs) pftypes
	else
		p

solveFor (p @ (Plaster rows size backtrack)) (f:fs) [] = do
	let backtrack = True
	p

solveFor (p @ (Plaster rows size backtrack)) [f @ (Field ftype oftype x y)] (pftype:pftypes) = do
	let ftype = pftype
	if isPlasterCorrect p then
		p
	else do
		let ftype = oftype
		solveFor p [f] pftypes

solve :: Plaster -> [Field] -> Plaster
solve p @ (Plaster rows size backtrack) ((f @ (Field ftype oftype x y)) : fs) 
	| (length fs) == 0 = do
		if oftype /= Empty then do
			if isPlasterCorrect p then
				p
			else do
				let backtrack = True
			...
			let possibleFtype = getPossibleFtype p f
			let p @ (Plaster rows size backtrack) = solveFor p [f] possibleFtype
	...
	| otherwise = do
		if oftype /= Empty then do
			...
			let possibleFtype = getPossibleFtype p f
			let p @ (Plaster rows size backtrack) = solveFor p (f:fs) possibleFtype
			if backtrack == False then
				p
			else do
				let ftype = oftype
